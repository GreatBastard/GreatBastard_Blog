...
- Changed the `ExternalLinks` configuration in `src/site.config.ts`. You should change your config like this:
  ...
  - Changed the comment component files out of package. The project now uses Twikoo through its CDN integration.
...
- Move some configuration items like `telegram` to new structure in `src/site.config.ts`.
